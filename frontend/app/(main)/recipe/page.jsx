...
<div className="lg:col-span-1 space-y-6">
  {/* Ingredients */}
  <div className="bg-white p-6 border-2 border-stone-200 lg:sticky lg:top-24 overflow-hidden">
      ...
      <div key={category} className="mb-6 last:mb-0">
        ...
        <ul className="space-y-2">
          {items.map((ingredient, i) => (
            <li
              key={i}
              className="flex justify-between items-start gap-4 text-stone-700 py-2 border-b border-stone-100 last:border-0 min-w-0"
            >
              {/* Added break-words and min-w-0 to ensure flex child wraps */}
              <span className="flex-1 leading-tight break-words min-w-0">
                {ingredient.item}
              </span>
              
              {/* FIXED: Added break-words and adjusted width to 50% for safety */}
              <span className="font-bold text-orange-600 text-sm text-right w-1/2 break-words leading-tight">
                {ingredient.amount}
              </span>
            </li>
          ))}
    ...
    {recipe.nutrition && (
      <div className="mt-6 pt-6 border-t-2 border-stone-200">
        ...
        >
          {/* Use grid-cols-2 and ensure children handle text wrapping */}
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Calories', value: recipe.nutrition.calories, color: 'orange' },
              { label: 'Protein', value: recipe.nutrition.protein, color: 'stone' },
              { label: 'Carbs', value: recipe.nutrition.carbs, color: 'stone' },
              { label: 'Fat', value: recipe.nutrition.fat, color: 'stone' }
            ].map((stat, idx) => (
              <div 
                key={idx} 
                className={`p-3 text-center border-2 flex flex-col justify-center min-h-[80px] overflow-hidden ${
                  stat.color === 'orange' ? 'bg-orange-50 border-orange-100' : 'bg-stone-50 border-stone-100'
                }`}
              >
                {/* Reduced font size from text-2xl to handle sentences, added break-words */}
                <div className={`font-bold leading-tight break-words mb-1 ${
                  stat.color === 'orange' ? 'text-orange-600 text-lg' : 'text-stone-900 text-lg'
                }`}>
                  {stat.value}
                </div>
                <div className="text-[10px] text-stone-500 font-bold uppercase tracking-wide mt-auto">
                  {stat.label}
                </div>
              </div>
            ))}
